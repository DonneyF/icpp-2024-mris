#
# Compare algorithms by sweeping the number of jobs
#
...
def main(downsample_factor, downsample_offset, run):
    ...
    df = pd.concat(dfs)
    df.to_parquet(Path(__file__).parent / f'results/3_scheduler_benchmark_jobs_{downsample_factor}_{run}.parquet')
