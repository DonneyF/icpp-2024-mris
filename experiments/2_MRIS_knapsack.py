#
# Compare the MRIS algorithm with a different knapsack solution
#
...
def main(downsample_factor, downsample_offset, run):
    ...
    df = pd.concat(dfs)
    df.to_parquet(Path(__file__).parent / f'results/2_MRIS_knapsack_{downsample_factor}_{run}.parquet')
