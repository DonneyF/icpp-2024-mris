#
# Compare the algorithms by sweeping the number of machines
#
...
def main(downsample_factor, downsample_offset, run, M):
    schedulers = [
        MRIS(sort='WSJF'),
    ...
    df = pd.concat(dfs)
    df.to_parquet(Path(__file__).parent / f'results/4_scheduler_benchmark_machines_{M}_{run}.parquet')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', dest='M', type=int, help="Number of machines")
    args = parser.parse_args()
    main(downsample_factor=args.downsample_factor, downsample_offset=args.downsample_offset, run=args.run, M=args.M)
